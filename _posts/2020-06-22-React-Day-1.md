...
따라서, 이 부분에 초점을 두고 공부를 하면 이해가 쉬울 듯 하다.  
...
### Directory 분석
Create React App을 통해 새로운 프로젝트를 생성하면 해당 폴더의 디렉토리는 크게 public과 src로 나눠진다.  
public은 index.html이 위치하는 곳.  
...
index.js에서 주목할 건 바로 `ReactDOM.render(<App />, document.getElementById('root'));`.  
...
```
이 App이라는 component, 즉 사용자 정의 태그의 실제 내용이다.  
